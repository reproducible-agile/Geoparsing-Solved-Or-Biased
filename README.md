...
To more easily run [Edinburgh Geoparser](https://www.ltg.ed.ac.uk/software/geoparser/), we split articles in LGL, GeoVirus, and WikToR into separate datasets, which can be found at [`data/evaluation-corpora/split-datasets/`](data/evaluation-corpora/split-datasets/). The scripts used is [`scripts/toponym-resolution-evaluation-corpora-splitting.ipynb`](scripts/toponym-resolution-evaluation-corpora-splitting.ipynb).
